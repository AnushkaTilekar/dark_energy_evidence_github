...
# Then, the program menitoned in this file builds a mathematical model of an expanding universe 
...
# called "plotting.py", to compare against our real data (i.e. the dataset obtained from the 
...
def predict_distance_modulus_with_dark_energy_presence(z, H0=67.66, Om0=0.3096):
    """
    Predict the distane modulus assuming a universe containing dark energy.
    ...
    This model assumes dark energy beahaves as a cosmological constant 
    ...
    NOTE: Om0 here prepresents the "total" matter density, including both- ordinary
    ...
    The default values here aare derived from the same cosmological parameters
    ...
    # Python's "astropy" library, has a built-in method called, "distmod()", in it's cosmology module's 
    # (i.e. "subpackage's") "FlatLambdaCDM" class.
    # The "distmod(z)" funciton tells our cosmology model, currently called, "our_custom_cosmology_model",
    # to calculate the distance modulus at each specified redshift value 'z' form the input redshift 
    # array. Remember, the redshift 'z' array being used here is take form the user's input dataset in the data_loader.py file.
    ...
    # displaying plots, in the file "plotting.py" to be precise, and also for keeping a check on 
    ...
    # But also, further in the package, in our calculations, we would need to use these obtained distance modulus numbers
    # without their units - i.e. in pure number form - for some mathematical procedures.
    # Hence, we now we use "valu()" funciton, which is a built-in function of the Pytohn's astropy lbrary.
    # This .value at the end strips away the extra unit-tracking information which astropy had attached internally above 
    # leaving us with plain numbers for the obtained distance modulus values for each given redshift value.
    # Here,
    # mu => "distance modulus"
    ...
    # This line sends the above obtained array of predicted distance modulus values (as pure numbers) back out to
    # whichever variable or funciton called this "predict_distance_modulus_with_dark_energy()" function.
    # Here,
    # mu => "distance modulus"
...
# Thus, in this ocde, "Om0 includes both ordinary matter and dark matter, along with dark energy".
